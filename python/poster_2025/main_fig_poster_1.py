...
tcurves = {}
for region in ['adn', 'lmn']:
    tuning_curves = nap.compute_tuning_curves(
        data.spikes.getby_category("location")[region],
        data.position['ry'],
        bins=60,
        range=(0, 2*np.pi),
        epochs=data.epochs['wake']
    )
    tuning_curves = smoothAngularTuningCurves(tuning_curves, 30, 3)
    mi = nap.compute_mutual_information(tuning_curves)
    tcurves[region.upper()] = tuning_curves.sel(unit=mi[mi['bits/spike']>0.1].dropna().index)
...
outergs = GridSpec(3, 1, figure=fig, height_ratios=[0.6, 0.2, 0.4], hspace=0.3)
...
for i, st in enumerate(["ADN", "LMN"]):
    ...
    tcurve = tcurves[st]
    peak = np.array([circmean(tcurve.coords[tcurve.dims[1]].values, tcurve[i].values) for i in range(tcurve.shape[0])])
    peak = pd.Series(index=tcurve.unit.values, data=peak)
    ...
    tc = tc / tc[:, tc.shape[1]//2]
    plot(tc.coords["0"].values, tc.values.T, linewidth=0.1, color=colors[st], alpha=0.4)
    plot(tc.coords["0"].values, tc.mean(dim="unit").values, linewidth=0.5, color=colors[st])
    xticks([])
    if i == 1:
    ...
    theta = peak.values
    radius = np.sqrt(np.max(tcurves[st].values, axis=0))
...
for i, e in enumerate(['sws', 'wak']):
    ...
    plot(y, x, 'o', markersize=2, color = colors2[i])
...
iui = data2['iui']
for i, e, c in zip([0,1], ['wak', 'sws'], [1, 0]):
    subplot(gs_iui[i,1])
    simpleaxis(gca())        
    # semilogx(iui[e], color='grey', alpha=0.5, linewidth=0.5)
    tmp = iui[e].mean(1)*100
    bar(tmp.index.values[0:-1], tmp.values[0:-1], width=np.diff(tmp.index.values), align='edge', facecolor = colors2[c], edgecolor=colors2[c])
    gca().set_xscale("log")
    ...
    # if i in [0, 1]:
        ...
        xlabel("Inter UFO intervals (s)")

#########################################
# LFP examples
#########################################

gs2 = gridspec.GridSpecFromSubplotSpec(1, 3, subplot_spec=outergs[2, 0], width_ratios=[0.3, 0.5, 0.1])
...
### LFP examples
gslfp = gridspec.GridSpecFromSubplotSpec(4, 1,
    subplot_spec=gs2[0, 1], hspace=0.05)
...
#
# gsmean3 = gridspec.GridSpecFromSubplotSpec(3, 1, subplot_spec=gs2[0, 2])
#
# bins = np.geomspace(1, 20, 100)
# # bins = np.linspace(0.5, 20, 100)
# # bins = np.logspace(0.5, 20, 100)
#
# pwr_ufo = np.array([np.histogram(data2['ufos'][s], bins)[0] for s in data2['ufos'].keys()]).T
# pwr_ctr = np.array([np.histogram(data2['ctrl'][s], bins)[0] for s in data2['ctrl'].keys()]).T
...
outergs.update(top=0.98, bottom=0.03, right=0.98, left=0.05)


savefig(
    os.path.expanduser("~") + r"/Dropbox/Applications/Overleaf/SFN 2025/figures/fig1.pdf",
