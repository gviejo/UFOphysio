...
outergs = GridSpec(2, 1, figure=fig, hspace=0.5, height_ratios=[0.5, 0.2])


gs_psb = gridspec.GridSpecFromSubplotSpec(4, 2, 
    subplot_spec=outergs[0,0], width_ratios=[0.3, 0.7], height_ratios=[0.2, 0.2, 0.2, 0.2], hspace=0.1
    )
...
for i, struct in enumerate(['adn', 'psb']):
    ...
    tcurves = {
        struct : nap.compute_tuning_curves(
            data.spikes.getby_category("location")[struct],
            data.position['ry'],
            bins=60,
            range=(0, 2*np.pi),
        ...
        "lmn" : nap.compute_tuning_curves(
            data.spikes.getby_category("location")['lmn'],
            data.position['ry'],
            bins=60,
            range=(0, 2*np.pi),
            epochs=data.epochs['wake'],
    ...
    grs['order'] = tcurves['lmn'].idxmax(dim="angle").values
    ...
    grs['order'] = np.argsort(tcurves[struct].idxmax(dim="angle").values)
    plot(grs.to_tsd("order"), '|', color=colors[structs[i].upper()], markersize=ms, markeredgewidth=mew)
    xlim(ep[0,0], ep[0,1])
    ylabel(structs[i].upper(), rotation=0, labelpad=5, y=0.1)
    xticks([])
    yticks([len(grs)-1], [len(grs)])
...
for i, e in enumerate(['wak', 'sws']):
    ...
    if i in [0, 2]:
        ylabel("Rate\n(norm.)", rotation = 0, labelpad=20, y=0.3)
    title(titles[i])
...
outergs.update(top=0.98, bottom=0.15, right=0.98, left=0.1)

savefig(
    os.path.expanduser("~") + r"/Dropbox/Applications/Overleaf/SFN 2025/figures/fig2.pdf",
    dpi=200,
    facecolor="white",
)
