...
for s in datasets:
# for s in ['LMN-ADN/B2000/B2001/B2001-210901']:
    print(s)
    ###############################################################################################
    # LOADING DATA
    ###############################################################################################
    ...
    # if (np.sum(spikes.location == "adn") > 5) and (np.sum(spikes.location == "lmn") > 5):
...
with open(os.path.join(save_dir, "ufo_reacs_LMN_ADN.pkl"), "wb") as f:
...
def draw_session_page(fig, s, gs_reac, gs_bot, n_max):
    ...
    # common y-limits for the reactivation panels across lmn/adn
...
pdf_path = os.path.expanduser("~/Dropbox/UFOPhysio/figures/UFO_reactivation_LMN_ADN.pdf")
