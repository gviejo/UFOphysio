...
for s in datasets[1:]:
	...
	# LOAD LFP
	############################################################
	dat_file 	= os.path.join(path,name+'_despike.dat')

	# ADN CHANNELS ONLY
	shank_to_channel = {i+1:shank_to_channel[i] for i in [7,8,9,10]}
	...
	# MEMORY MAP
	fp = np.memmap(dat_file, np.int16, 'r', shape = (n_samples, n_channels))
	...
	# channels_adn = np.hstack((shank_to_channel[8],shank_to_channel[9]))
	# channels_ctr = np.hstack((shank_to_channel[10],shank_to_channel[11]))

	channels_adn = shank_to_channel[8][0:5]
	channels_ctr = shank_to_channel[11][0:5]

	for ch in channels_adn:
		print(ch)
		##################################################################################################
		# LOADING LFP
		##################################################################################################	
	...
	nSS[:,0] /= len(channels_adn)
	SS[:,0] /= len(channels_adn)


	for ch in channels_ctr:
		print(ch)
		##################################################################################################
		# LOADING LFP
		##################################################################################################	
	...
	nSS[:,1] /= len(channels_ctr)
	SS[:,1] /= len(channels_ctr)
