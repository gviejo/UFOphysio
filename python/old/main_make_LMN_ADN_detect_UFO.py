...
datfile = os.path.join(path, name+'_despike.dat')
...
nSS[:,0] /= len(shank_to_channel[4])
SS[:,0] /= len(shank_to_channel[4])

for i, sh in enumerate([6]):
...
nSS[:,1] /= len(shank_to_channel[4])*2
SS[:,1] /= len(shank_to_channel[4])*2
